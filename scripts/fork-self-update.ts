...
export function listTrackedDirtyEntries(entries: ReadonlyArray<string>): ReadonlyArray<string> {
  return entries.filter((entry) => !entry.startsWith("?? "));
}

export function findMissingConnectPublicConfig(
  environment: Readonly<Record<string, string | undefined>>,
): ReadonlyArray<string> {
  const connectConfig = resolvePublicConfig(environment);
  return [
...
async function resolveRepository(inputPath: string): Promise<string> {
  const repoPath = await NodeFSP.realpath(NodePath.resolve(inputPath));
  if (!NodeFS.existsSync(NodePath.join(repoPath, ".git"))) {
...
async function inspect(repoPath: string, fetch: boolean) {
  ...
  const installedCommit = await NodeFSP.readFile(
    NodePath.join(repoPath, ".t3", "fork-updater", "installed-commit"),
    "utf8",
  )
    .then((value) => value.trim())
    .catch(() => "");
  const installedTrack = await NodeFSP.readFile(
    NodePath.join(repoPath, ".t3", "fork-updater", "installed-track"),
    "utf8",
  )
    .then((value) => value.trim())
    .catch(() => "unknown");
  const dirtyEntries = status.stdout.length === 0 ? [] : status.stdout.split("\n");
  const trackedDirtyEntries = listTrackedDirtyEntries(dirtyEntries);
...
async function verifyForkBundle(appPath: string, repoPath: string) {
  if (!REPLACEABLE_APP_NAMES.has(NodePath.basename(appPath))) {
    throw new ForkUpdateError("wrong-app-name", "The build must use a known T3 Code app name.");
  }
  const packageArchive = await NodeFSP.readFile(
    NodePath.join(appPath, "Contents", "Resources", "app.asar"),
  );
...
async function build(
  ...
  reportProgress(20);
  const buildRoot = NodePath.join(
    outputRoot,
    ".t3",
    "fork-updater",
    `build-${commit.slice(0, 12)}`,
  );
  await NodeFSP.rm(buildRoot, { recursive: true, force: true });
  ...
  // Fork builds run outside the official release workflow, so carry the
  // repository's public Connect configuration into both personal builds and
  // detached official-track worktrees. Building without it silently removes
  // every T3 Connect sign-in surface from the packaged app.
  const buildEnv = loadRepoEnv({ repoRoot: sourceRepository });
  const missingConnectConfig = findMissingConnectPublicConfig(buildEnv);
  if (missingConnectConfig.length > 0) {
    throw new ForkUpdateError(
      "connect-public-config-missing",
      "Personal updates require T3 Connect public configuration; the installed app was not replaced.",
      { missing: missingConnectConfig },
    );
  }
  const rustupBin = "/opt/homebrew/opt/rustup/bin";
  if (NodeFS.existsSync(rustupBin)) {
  ...
  await NodeFSP.writeFile(
    NodePath.join(appPath, "Contents", "Resources", "app-update.yml"),
    "provider: github\nowner: pingdotgg\nrepo: t3code\n",
  );
...
export const TRACK_OVERLAY_PATHS = [
...
async function buildTrack(repoPath: string, track: "alpha" | "nightly" | "equinox") {
  ...
  const trackedDirtyEntries = listTrackedDirtyEntries(inspected.dirtyEntries);
  ...
  const worktreePath = NodePath.join(repoPath, ".t3", "fork-updater", `track-source-${track}`);
  await git(repoPath, ["worktree", "remove", "--force", worktreePath], true);
  await NodeFSP.rm(worktreePath, { recursive: true, force: true });
...
async function update(repoPath: string) {
  ...
  const trackedDirtyEntries = listTrackedDirtyEntries(before.dirtyEntries);
  if (trackedDirtyEntries.length > 0) {
    throw new ForkUpdateError(
      "dirty-checkout",
  ...
  // Every fork commit is intentional. In particular, conflict-to-chat routing
  // remains a separate commit from the Equinox feature commit, so updates
  // rebase and push the existing history without consolidating it.
...
async function install(
  ...
  const builtApp = await NodeFSP.realpath(NodePath.resolve(builtAppInput));
  const targetApp = NodePath.resolve(targetAppInput);
  const previousApp = previousAppInput ? NodePath.resolve(previousAppInput) : targetApp;
  if (
    !REPLACEABLE_APP_NAMES.has(NodePath.basename(builtApp)) ||
    !REPLACEABLE_APP_NAMES.has(NodePath.basename(targetApp)) ||
    !REPLACEABLE_APP_NAMES.has(NodePath.basename(previousApp))
  ) {
    throw new ForkUpdateError(
      "unsafe-app-target",
  ...
  if (targetApp !== previousApp && NodeFS.existsSync(targetApp) && NodeFS.existsSync(previousApp)) {
    throw new ForkUpdateError(
      "ambiguous-app-target",
      "Both the fork target and the previous T3 Code app exist; refusing to choose one.",
    );
  }
  if (!builtApp.includes(`${NodePath.join(".t3", "fork-updater")}/`)) {
    throw new ForkUpdateError("untrusted-build", "The app was not produced by the fork updater.");
  }
  const marker = `${NodePath.join(".t3", "fork-updater")}/`;
  const repoPath = NodePath.resolve(builtApp.slice(0, builtApp.indexOf(marker)));
  ...
  const stagedApp = NodePath.join(targetDirectory, `.T3 Code.update-${process.pid}.app`);
  const backupApp = NodePath.join(targetDirectory, `.T3 Code.previous-${process.pid}.app`);
  await NodeFSP.rm(stagedApp, { recursive: true, force: true });
  await NodeFSP.rm(backupApp, { recursive: true, force: true });
  ...
  const installedApp = NodeFS.existsSync(targetApp) ? targetApp : previousApp;
  if (NodeFS.existsSync(installedApp)) {
    await NodeFSP.rename(installedApp, backupApp);
  }
  try {
    await NodeFSP.rename(stagedApp, targetApp);
    ...
    const installedCommitPath = NodePath.join(repoPath, ".t3", "fork-updater", "installed-commit");
    await NodeFSP.mkdir(NodePath.dirname(installedCommitPath), { recursive: true });
    await NodeFSP.writeFile(installedCommitPath, `${installedCommit.stdout}\n`);
    await NodeFSP.writeFile(
      NodePath.join(repoPath, ".t3", "fork-updater", "installed-track"),
      `${track}\n`,
    );
    ...
    if (NodeFS.existsSync(backupApp) && !NodeFS.existsSync(targetApp)) {
      await NodeFSP.rename(backupApp, installedApp);
    }
    throw error;
  }
