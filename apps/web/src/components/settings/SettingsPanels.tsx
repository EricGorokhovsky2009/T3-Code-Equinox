    ...
    void handleNewThread(projectRef, { envMode: "local", replace: true })
      .then(() => {
        ...
        store.setPrompt(
          draft.draftId,
          [
            "Resolve the interrupted upstream rebase in this T3 Code fork.",
            "",
            `Repository: ${updateState.sourceRepositoryPath}`,
            "Preserve our custom features while integrating the official upstream changes.",
            "Inspect the rebase state, resolve every conflict, continue the rebase, run focused verification, rebuild the fork app, and push main to origin with lease protection.",
            conflictList ? `\nConflicted files:\n${conflictList}` : "",
          ]
            .filter(Boolean)
            .join("\n"),
