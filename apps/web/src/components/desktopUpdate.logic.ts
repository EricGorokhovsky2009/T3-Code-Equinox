...
export function getDesktopUpdateButtonTooltip(state: DesktopUpdateState): string {
  ...
  if (sourceUpdate && (state.sourceConflictFiles?.length ?? 0) > 0) {
    return "The interrupted fork rebase needs conflict resolution before the update can continue";
  }
  if (state.status === "available") {
    if (sourceUpdate) {
      if ((state.sourceBehindCount ?? 0) === 0) {
        return "Your GitHub changes are ready to build as a Personal Update";
      }
      return `${state.sourceBehindCount ?? 0} official change${
        state.sourceBehindCount === 1 ? "" : "s"
      } ready to Sync With T3 Code`;
    }
    return `Update ${state.availableVersion ?? "available"} ready to download`;
  }
  if (state.status === "downloading") {
    if (sourceUpdate) {
      return (state.sourceBehindCount ?? 0) > 0
        ? "Syncing official T3 Code changes and rebuilding Equinox"
        : "Building your Personal Update from GitHub";
    }
    const progress =
