...
export type ForkUpdateCommandResult = typeof ForkUpdateCommandResult.Type;
...
export function reduceDesktopUpdateStateOnForkCommand(
  ...
  if (result.status === "available") {
    return {
      ...
      message: result.dirty
        ? "Commit tracked source changes before updating. Preserved untracked Equinox artwork does not block updates."
        : (result.behindCount ?? 0) > 0
          ? "Official T3 Code changes are ready to merge into your fork."
          : "Your fork changes are ready to rebuild and install.",
      errorContext: null,
      canRetry: false,
    };
  }
  ...
  return {
    ...
    message:
      result.dirty === true
        ? "Your fork is current with upstream. Tracked source changes have not been built yet."
        : null,
    errorContext: null,
    canRetry: false,
  };
...
function resolveExecutable(name: string, homeDirectory: string): string | null {
  const candidates = [
    ...(process.env.PATH ?? "")
      ...
      .map((entry) => NodePath.join(entry, name)),
    NodePath.join(homeDirectory, ".bun", "bin", name),
    NodePath.join("/opt/homebrew/bin", name),
    NodePath.join("/usr/local/bin", name),
  ];
  return (
    candidates.find((candidate) => candidate.length > 0 && NodeFS.existsSync(candidate)) ?? null
  );
}
...
function runForkUpdateCommand(input: {
  ...
  return new Promise((resolve, reject) => {
    const child = NodeChildProcess.spawn(input.bunPath, [input.scriptPath, ...input.args], {
      cwd: input.cwd,
      env: process.env,
...
export const make = Effect.gen(function* () {
  ...
  const runForkUpdater = Effect.fn("desktop.updates.runForkUpdater")(function* (
    ...
    if (!bunPath) {
      return {
        ...
        code: "bun-not-found",
      } satisfies ForkUpdateCommandResult;
    }
  ...
  const applyForkCommandState = (
    ...
    updateState((current) => reduceDesktopUpdateStateOnForkCommand(current, result, checkedAt));
  ...
  const buildForkSourceUpdate = Effect.fn("desktop.updates.buildForkSource")(function* (
    ...
    if (
      ...
      currentState.updateKind !== "source"
    ) {
